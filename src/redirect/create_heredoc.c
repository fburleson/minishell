...
/*   Updated: 2023/10/10 17:32:25 by joel             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	copy_to_heredoc(t_iofile *file, int heredoc)
{
	...
	src = ft_open(file->path, O_RDONLY);
	if (src == -1)
		return ;
...
int	create_heredoc(t_iofile *file)
{
	...
	heredoc = ft_mopen(tmpfile_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (heredoc == -1)
		return (-1);
	copy_to_heredoc(file, heredoc);
	close(heredoc);
	return (ft_open(tmpfile_path, O_RDONLY));
}
